module SongkickHelper
  ...
  def self.get_events
    ...
    pages = songkick.events(min_date: 12.hours.ago.strftime("%Y-%m-%d"), max_date: 12.hours.from_now.strftime("%Y-%m-%d"),location: "sk:9179", page: 1000)
    total_pages = (pages.total_entries / 50) + 1 # tells how many pages of events there are
    i = 1
    artists = []
    events = []
    while i <= total_pages
      response = songkick.events(min_date: 12.hours.ago.strftime("%Y-%m-%d"), max_date: 12.hours.from_now.strftime("%Y-%m-%d"),location: "sk:9179", page: i)
